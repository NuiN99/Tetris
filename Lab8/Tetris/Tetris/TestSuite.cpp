...
void TestSuite::testTetrominoClass()
{
	...
	assert(t._blockLocs.size() == blockcount &&
		"default Tetromino has no __blockLocs - likely because no default set in constructor");

	t.setShape(TetShape::S);
	assert(t._blockLocs.size() == blockcount && "Tetromino shape size should be: 4");
	...
	t._blockLocs.clear();
	t._blockLocs.push_back(Point(1, 2));
	t.rotateClockwise();
	assert(t._blockLocs[0].getX() == 2 && t._blockLocs[0].getY() == -1 && "Tetromino::rotateClockwise() failed");
	t.rotateClockwise();
	assert(t._blockLocs[0].getX() == -1 && t._blockLocs[0].getY() == -2 && "Tetromino::rotateClockwise() failed");
	t.rotateClockwise();
	assert(t._blockLocs[0].getX() == -2 && t._blockLocs[0].getY() == 1 && "Tetromino::rotateClockwise() failed");
	t.rotateClockwise();
	assert(t._blockLocs[0].getX() == 1 && t._blockLocs[0].getY() == 2 && "Tetromino::rotateClockwise() failed");
 ...
 void TestSuite::testGridTetrominoClass()
{
	...
	// test get_blockLocsMappedToGrid()
	gt._blockLocs = { Point(1,2) };
	gt.setGridLoc(5, 5);

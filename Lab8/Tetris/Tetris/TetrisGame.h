...
public:
	...
	static const float FASTEST_SPEED_SCORE; // the score where the game will be the fastest eg
...
public:
	...
	TetrisGame(sf::RenderWindow& window, sf::Sprite& blockSprite, const Point& gameboardOffset, const Point& nextShapeOffset,
		const sf::Sound& blockDrop, const sf::Sound& blockRotate, const sf::Sound& gameOver, const sf::Sound& levelUp);
	...
	// - param 1: float deltaTime
	// return: nothing
	void processGameLoop(float deltaTime);
...
private:
	...
	// - param 1: GridTetromino shape
	// - return: nothing;
	void drop(GridTetromino& shape) const;
	...
	// return: nothing
	void drawBlock(Point topLeft, int xOffset, int yOffset, TetColor color, int transparency = 255) const;
	...
	// return: nothing
	void drawTetromino(GridTetromino& tetromino, Point topLeft, int transparency = 255) const;


	// copy the currentShape
	// set the position
	// drop it
	// draw it
